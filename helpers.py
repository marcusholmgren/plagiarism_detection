...
def train_test_dataframe(clean_df: pd.DataFrame, random_seed=100) -> pd.DataFrame:
...
def process_file(file: io.FileIO):
    ...
    all_text = re.sub(r"\n", " ", all_text)
    all_text = re.sub(r"\W{2,3}", " ", all_text)
...
def create_text_column(df: pd.DataFrame, file_directory='data/') -> pd.DataFrame:
